...
import Sidebar from '@/components/sampletools/Sidebar'
...
export default function PDFtoCSV() {
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      // For now, we'll use a simplified approach without the API client
      // The API client can be added later if needed
      const formData = new FormData()
      formData.append('file', file)

      const apiUrl = process.env.NODE_ENV === 'development'
        ? '/api/pdf-csv/api/jobs'
      ...
      if (uploadResponse.downloadUrl) {
          ...
          while (attempts < maxAttempts) {
            ...
            try {
              const statusUrl = process.env.NODE_ENV === 'development'
                ? `/api/pdf-csv/api/jobs/${jobId}/status`
                : `${BACKEND_URL}/api/jobs/${jobId}/status`

              const statusResponse = await fetch(statusUrl)
              const statusData = await statusResponse.json()

              if (statusData.status === 'completed' && statusData.downloadUrl) {
                return { ready: true, downloadUrl: statusData.downloadUrl }
        ...
        const finalStatus = await pollJobStatus(uploadResponse.jobId)
        ...
        if (finalStatus.ready && finalStatus.downloadUrl) {
          ...
          // Get download URL
          const downloadUrlEndpoint = process.env.NODE_ENV === 'development'
            ? `/api/pdf-csv/api/jobs/${uploadResponse.jobId}/download-url`
            : `${BACKEND_URL}/api/jobs/${uploadResponse.jobId}/download-url`
